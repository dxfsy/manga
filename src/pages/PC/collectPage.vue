...
export default {
  ...
  created() {
    ...
    getCollectList({ username: user.username }).then((res) => {
  ...
  methods: {
    ...
    search: _.throttle(
      function (key) {
        let user = getLocalStorage("user") || getSessionStorage("user");
        getCollectSearchList({ username: user.username, key }).then((res) => {
    ...
    async remove() {
      let user = getLocalStorage("user") || getSessionStorage("user");
      let res = await removeCollect({username:user.username,list:this.selectList})
      this.showToast(res.data.data.message)
